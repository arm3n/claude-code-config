...
8. GEMINI VERIFY (split-brain, three calls — Actuary FIRST): **(a) Actuary FIRST** — runs BEFORE Blind and Auditor. Separate `gemini-search` call that sees Claude's synthesis but uses locked adversarial prompt (see `/gemini` skill). Must name simplest alternative, #1 over-engineering risk, and critical dependency failure. "Looks good" is invalid output. **(b) Blind** — `gemini-search` with original claims converted to neutral questions, independent research. **(c) Auditor** — `gemini-search` with atomic claims from Claude's synthesis, SAFE protocol (verbatim quotes or UNVERIFIED). Never combine fact-checking and divergent critique in one call. Never `gemini-analyze-text` for any. **After all 3 calls, output a verification audit table** showing: call role, actual query sent, key finding. This is the user's visibility into whether calls were relabeled. See `/gemini` skill for full protocol.
