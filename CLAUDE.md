...
8. GEMINI VERIFY (split-brain, three calls): (a) **Blind** — `gemini-search` with original claims converted to neutral questions, independent research. (b) **Auditor** — `gemini-search` with atomic claims from Claude's synthesis, SAFE protocol (verbatim quotes or UNVERIFIED). (c) **Actuary** — separate `gemini-search` call: "What secondary effects, minority actors, or alternative mechanisms are completely ignored by this analysis?" Never combine fact-checking and divergent critique in one call. Never `gemini-analyze-text` for any. See `/gemini` skill for full protocol.
